...
from utils.feedingHabitat import feeding_dict
import os
import numpy as np
...
@app.route('/feeding-habitat', methods=['POST'])
def feeding_recommend():
    animal_name = str(request.form['specie'])
    weight = int(request.form['weight'])
    if animal_name == "Goats":
        key1 = "goatfood"
    ...
    response1 = Markup(str(feeding_dict[key1]))
    return render_template('FeedHabitat-Result.html', recommendation=response1)
...
@app.route("/FeedingRecommendation.html")
def feeding():
    return render_template("FeedingRecommendation.html")
